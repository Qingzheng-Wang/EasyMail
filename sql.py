...
class SQL:  # 数据存储类
    ...
    def create_sql(name):
        try:
            ...
            print(f'数据库连接失败：{e}')
        cursor = conn.cursor()
        sql_init = "create table  %s (sender char(80), receiver char(50), topic char(50), uid char(50),  \
    ...
    @staticmethod
    def search_sql(sender, receiver, topic,uid):
        try:
            conn = pymysql.connect(host='localhost',user='root',password='4268',database='mail')
        except Exception as e:
            print(f'数据库连接失败：{e}')
    ...
    @staticmethod
    def add_sql(sender, receiver, topic,uid):
        try:
            conn = pymysql.connect(host='localhost',user='root',password='4268',database='mail')
        except Exception as e:
            print(f'数据库连接失败：{e}')
        cursor = conn.cursor()
        sql_add = "INSERT INTO Mail VALUES ('%s','%s','%s','%s')" % (sender, receiver, topic, uid)
